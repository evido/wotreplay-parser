...
MainWindow::MainWindow(QWidget *parent)
  ...
  QStringList list = (QStringList() << "png" << "heatmap" << "team-heatmap"
                                    << "class-heatmap");
...
std::unique_ptr<image_writer_t>
MainWindow::create_writer(const std::string &path) const {
  ...
  if (type == "png") {
    ...
    heatmap_writer_t *_specific =
        dynamic_cast<heatmap_writer_t *>(writer.get());
    _specific->mode = type == "team-heatmap" ? heatmap_mode_t::team
                                             : heatmap_mode_t::combined;
    float lowerBound =
        ui->lowerBoundSlider->value() / (ui->lowerBoundSlider->maximum() * 1.f);
    float upperBound =
        ui->upperBoundSlider->value() / (ui->upperBoundSlider->maximum() * 1.f);
    _specific->bounds = std::make_tuple(lowerBound, upperBound);
  } else if (type == "class-heatmap") {
    writer.reset(new class_heatmap_writer_t());
    class_heatmap_writer_t *_specific =
        dynamic_cast<class_heatmap_writer_t *>(writer.get());
    float lowerBound =
        ui->lowerBoundSlider->value() / (ui->lowerBoundSlider->maximum() * 1.f);
    float upperBound =
        ui->upperBoundSlider->value() / (ui->upperBoundSlider->maximum() * 1.f);
    ...
    std::vector<draw_rule_t> rules =
        parse_draw_rules(ui->rulesTextEdit->toPlainText().toUtf8().constData());
...
void MainWindow::on_showButton_clicked() {
  ...
  auto dim = data.shape();
  if (!image_data)
    image_data.reset(new uint32_t[dim[0] * dim[1]]);
  for (size_t i = 0; i < dim[0]; ++i) {
    for (size_t j = 0; j < dim[1]; ++j) {
      uint32_t color =
          qRgba(data[i][j][0], data[i][j][1], data[i][j][2], data[i][j][3]);
      image_data[i * dim[1] + j] = color;
    }
  }

  QImage image((const uchar *)image_data.get(), dim[0], dim[1],
               dim[1] * sizeof(uint32_t), QImage::Format_ARGB32);
...
void MainWindow::on_browseInputButton_clicked() {
  ...
  QStringList filters;
  filters << tr("World of Tanks Replays (*.wotreplay)")

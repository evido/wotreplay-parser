...
void class_heatmap_writer_t::set_draw_rules(
...
void class_heatmap_writer_t::init(const wotreplay::arena_t &arena,
...
int class_heatmap_writer_t::get_class(const game_t &game,
                                      const packet_t &packet) const {
...
void class_heatmap_writer_t::finish() {
  ...
  for (int k = 0; k < class_count; k += 1) {
    std::tie(min[k], max[k]) =
        get_bounds(positions[k], std::get<0>(bounds), std::get<1>(bounds));
  }
  ...
  for (int i = 0; i < image_width; i += 1) {
    for (int j = 0; j < image_height; j += 1) {
      ...
      for (int k = 1; k < class_count; k += 1) {
        double _a =
            clamp((positions[k][i][j] - min[k]) / (max[k] - min[k]), 0.0, 1.0);
        if (_a > a) {
          a = _a;
          ix = k;
        }
      ...
      if (!no_basemap) {
        a *= (c & 0xFF) / 255.;
        result[i][j][0] =
            mix(result[i][j][0], result[i][j][0], 1. - a, (c >> 24) & 0xFF, a);
        result[i][j][1] =
            mix(result[i][j][1], result[i][j][1], 1. - a, (c >> 16) & 0xFF, a);
        result[i][j][2] =
            mix(result[i][j][2], result[i][j][2], 1. - a, (c >> 8) & 0xFF, a);
      } else {

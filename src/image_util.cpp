...
static_assert(sizeof(png_byte) <= sizeof(uint8_t),
              "Unexpected definition of png_byte");
...
void wotreplay::read_png(std::istream &is,
...
bool wotreplay::write_png(std::ostream &os,
                          boost::multi_array<uint8_t, 3> &image) {
  png_structp png_ptr =
  ...
  png_set_IHDR(png_ptr, info_ptr, static_cast<uint32_t>(width),
...
void wotreplay::get_row_pointers(boost::multi_array<uint8_t, 3> &image,
                                 std::vector<png_bytep> &row_pointers) {
  const size_t *shape = image.shape();
...
void wotreplay::read_mini_map(const std::string &map_name,
                              const std::string &game_mode,
                              boost::multi_array<uint8_t, 3> &base) {
  std::string mini_map =
      (boost::format("maps/no-border/%1%_%2%.png") % map_name % game_mode)
          .str();
...
void wotreplay::resize(boost::multi_array<uint8_t, 3> &original, int width,
                       int height, boost::multi_array<uint8_t, 3> &result) {
    ...
    for (int x = 0; x < width; ++x) {
      ...
      for (int c = 0; c < shape[2]; ++c) {
        result[y][x][c] = original[yy][xx][c] * (1 - dy) * (1 - dx) +
                          original[std::min(yy + 1, (int)shape[0] - 1)][xx][c] *
                              dy * (1 - dx) +
                          original[std::min(yy + 1, (int)shape[0] - 1)]
                                  [std::min(xx + 1, (int)shape[1] - 1)][c] *
                              dy * dx +
                          original[yy][std::min(xx + 1, (int)shape[1] - 1)][c] *
                              (1 - dy) * dx;
      }
    }
  }

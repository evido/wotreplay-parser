...
bool wotreplay::write_png(std::ostream &os, boost::multi_array<uint8_t, 3> &image) {
    ...
    png_set_IHDR(png_ptr, info_ptr, static_cast<uint32_t>(width), static_cast<uint32_t>(height), 8, alpha ? PNG_COLOR_TYPE_RGBA : PNG_COLOR_TYPE_RGB,
...
int wotreplay::mix(int v0, int v1, float a1, int v2, float a2) { return (1 - a2) * ((1 - a1) * v0 + a1 * v1) + a2 * v2; }
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            for (int c = 0; c < shape[2]; ++c) {
                result[y][x][c] = original[yy][xx][c] * (1 - dy) * (1 - dx) + original[std::min(yy + 1, (int)shape[0] - 1)][xx][c] * dy * (1 - dx) +

...
template <typename T> static void debug_stream_content(const std::string &file_name, T begin, T end) {
    ...
    if (raw) {
        ...
        if (debug) {
            for (int i = 0; i < data_blocks.size(); ++i) {
                debug_stream_content((boost::format("data-block-%1%.dat") % i).str(), data_blocks[i].begin(), data_blocks[i].end());
            }
        }
...
bool parser_t::is_legacy_replay(const buffer_t &buffer) const { return buffer.size() >= 10 && buffer[8] == 0x78 && buffer[9] == 0xDA; }
...
void parser_t::extract_replay(const unsigned char *begin, const unsigned char *end, buffer_t &replay) {
    ...
    z_stream strm = {const_cast<z_const Bytef *>(begin), static_cast<uInt>(end - begin)};
...
void parser_t::read_player_info(game_t &game) {
    ...
    const auto player_account_id = root[0]["personal"]["avatar"]["accountDBID"].asUInt();
    ...
    for (auto it = vehicles.begin(); it != vehicles.end(); ++it) {
        ...
        player.name = players[boost::lexical_cast<std::string>(player.player_id)]["name"].asString();
...
bool wotreplay::is_replayfile(const boost::filesystem::path &p) { return is_regular_file(p) && p.extension() == ".wotreplay"; }

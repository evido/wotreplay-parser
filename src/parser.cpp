...
static void debug_stream_content(const std::string &file_name, T begin, T end) {
  ...
  std::copy(
      begin, end,
      std::ostream_iterator<typename std::iterator_traits<T>::value_type>(os));
  os.close();
}
...
static std::map<game_title_t, std::array<unsigned char, 16>> encryption_keys = {
    {game_title_t::world_of_tanks,
     {0xDE, 0x72, 0xBE, 0xA0, 0xDE, 0x04, 0xBE, 0xB1, 0xDE, 0xFE, 0xBE, 0xEF,
      0xDE, 0xAD, 0xBE, 0xEF}},
    {game_title_t::world_of_warships,
     {0x29, 0xB7, 0xC9, 0x09, 0x38, 0x3F, 0x84, 0x88, 0xFA, 0x98, 0xEC, 0x4E,
      0x13, 0x19, 0x79, 0xFB}}};

parser_t::parser_t(std::unique_ptr<packet_reader_t> &&packet_reader,
                   load_data_mode_t load_data_mode, bool debug)
    : debug(debug), load_data_mode(load_data_mode),
      packet_reader(std::move(packet_reader)) {
  // empty
...
void parser_t::parse(buffer_t &buffer, wotreplay::game_t &game, bool raw) {
  if (raw) {
    ...
    if (data_blocks.size() < 2) {
      std::string message(
          (boost::format("Unexpected number of data blocks (%1%).") %
           data_blocks.size())
              .str());
    ...
    std::copy(data_blocks[1].begin(), data_blocks[1].end(),
              player_info.begin());
    ...
    std::copy(data_blocks.back().begin(), data_blocks.back().end(),
              raw_replay.begin());
    ...
    uint32_t decompressed_size =
        *reinterpret_cast<const uint32_t *>(&raw_replay[0]);
    uint32_t compressed_size =
        *reinterpret_cast<const uint32_t *>(&raw_replay[4]);
    decrypt_replay(raw_replay.data() + 8, raw_replay.data() + raw_replay.size(),
                   key);

    game.replay.resize(decompressed_size, 0);
    extract_replay(raw_replay.data() + 8,
                   raw_replay.data() + 8 + compressed_size, game.replay);
    debug_stream_content("replay.dat", game.replay.begin(), game.replay.end());

    uint32_t version_string_sz =
        get_field<uint32_t>(game.replay.begin(), game.replay.end(), 12);
...
void parser_t::set_raw_offset(int raw_offset) { this->raw_offset = raw_offset; }

void parser_t::set_debug(bool debug) { this->debug = debug; }

bool parser_t::get_debug() const { return debug; }

bool parser_t::is_legacy_replay(const buffer_t &buffer) const {
  return buffer.size() >= 10 && buffer[8] == 0x78 && buffer[9] == 0xDA;
}

bool parser_t::is_legacy() const { return legacy; }

void parser_t::decrypt_replay(unsigned char *begin, unsigned char *end,
  ...
  while (pin < (end - begin)) {
    ...
    std::transform(previous, previous + decrypted_len, decrypted, decrypted,
...
uint32_t parser_t::get_data_block_count(const buffer_t &buffer) const {
  ...
  const uint32_t *db_cnt =
      reinterpret_cast<const uint32_t *>(&buffer[db_cnt_offset]);
  return *db_cnt;
}

void parser_t::extract_replay(const unsigned char *begin,
                              const unsigned char *end, buffer_t &replay) {
  debug_stream_content("replay-c.dat", begin, end);

  z_stream strm = {const_cast<z_const Bytef *>(begin),
  ...
  if (ret != Z_OK) {
    ...
    msg << __func__ << ": inflateInit() failed!";
  ...
  if (ret != Z_STREAM_END) {
    std::stringstream msg;
    msg << __func__ << ": infate() failed!\n";
    throw std::runtime_error(msg.str());
  }
}

void parser_t::get_data_blocks(buffer_t &buffer,
                               std::vector<slice_t> &data_blocks) const {
  if (buffer.size() == 0) {
  ...
  for (uint32_t ix = 0; ix < nr_data_blocks; ++ix) {
    ...
    const uint32_t *block_size =
        reinterpret_cast<const uint32_t *>(&buffer[data_block_sz_offset]);
  ...
  // last slice contains encrypted / compressed game replay, seperated by 8
  // bytes with unknown content uint32_t decompressed_size =
  // *reinterpret_cast<const uint32_t *>(&buffer[data_block_sz_offset]);
  uint32_t compressed_size =
      *reinterpret_cast<const uint32_t *>(&buffer[data_block_sz_offset + 4]);
...
void parser_t::read_arena_info(game_t &game) {
  ...
  get_arena(map_name, game.arena,
            load_data_mode == load_data_mode_t::on_demand);
}

...
static bool is_not_empty(const packet_t &packet) {
  ...
  std::set<property_t> standard_properties = {
      property_t::clock, property_t::player_id, property_t::type,
      property_t::sub_type, property_t::length};
...
void generate_minimap(const arena_t &arena, const std::string &game_mode,
                      int team_id, const std::string &output) {
  image_writer_t writer;

  boost::format file_name_format("%1%/%2%_%3%_%4%.png");
  std::string file_name =
      (file_name_format % output % arena.name % game_mode % team_id).str();

  logger.writef(log_level_t::info, "Generating mini map (%1%, %2%, %3%, %4%)",
                output, arena.name, game_mode, team_id);
  ...
  try {
    ...
    logger.writef(log_level_t::error, "Failed to create %1%: %2%", file_name,
                  exc.what());
  }
}
...
int create_minimaps(const po::variables_map &vm, const std::string &output,
  ...
  if (vm.count("output") == 0) {
    logger.write(wotreplay::log_level_t::error,
                 "parameter output is required to use this mode");
    return EX_USAGE;
  }
...
void apply_settings(heatmap_writer_t *const writer,
...
void apply_settings(class_heatmap_writer_t *const writer,
                    const po::variables_map &vm) {
...
void apply_settings(animation_writer_t *const writer,
                    const po::variables_map &vm) {
...
std::unique_ptr<writer_t> create_writer(const std::string &type,
                                        const po::variables_map &vm) {
  ...
  if (type == "png") {
    ...
    logger.writef(
        log_level_t::error,
        "Invalid output type (%1%), supported types: png, gif, json, heatmap, "
...
#ifdef ENABLE_TBB
int process_replay_directory(const po::variables_map &vm,
  ...
  auto f_generate_image = [&type, &vm](game_t *game_) -> image_writer_t * {
    ...
    if (writer && dynamic_cast<image_writer_t *>(writer.get())) {
      std::unique_ptr<image_writer_t> image_writer(
          static_cast<image_writer_t *>(writer.release()));
  ...
  auto f_merge_image = [&writers](image_writer_t *writer_) {
    ...
    auto key =
        std::make_tuple(writer->get_arena().name, writer->get_game_mode());
  ...
  tbb::parallel_pipeline(
      tokens, tbb::make_filter<void, std::string>(tbb::filter::serial_in_order,
                                                  f_generate_replay_paths) &
                  tbb::make_filter<std::string, game_t *>(tbb::filter::parallel,
                                                          f_parse_replay) &
                  tbb::make_filter<game_t *, image_writer_t *>(
                      tbb::filter::parallel, f_generate_image) &
                  tbb::make_filter<image_writer_t *, void>(
                      tbb::filter::serial_out_of_order, f_merge_image));

  typedef std::map<std::tuple<std::string, std::string>,
                   image_writer_t *>::iterator::value_type item_t;
  tbb::parallel_do(writers.begin(), writers.end(), [&output](const item_t &it) {
    path file_name =
        path(output) / (boost::format("%s_%s.png") % std::get<0>(it.first) %
                        std::get<1>(it.first))
                           .str();
...
int process_replay_directory(const po::variables_map &vm,
  ...
  if (!(vm.count("type") > 0 && vm.count("input") > 0)) {
    logger.write(wotreplay::log_level_t::error,
                 "parameters type and input are required to use this mode\n");
    return EX_USAGE;
  }

  parser_t parser(
      std::move(std::unique_ptr<packet_reader_t>(new packet_reader_80_t())),
      load_data_mode_t::bulk);
  ...
  for (auto it = directory_iterator(input); it != directory_iterator(); ++it) {
    ...
    std::string name =
        (boost::format("%s_%s") % game.get_map_name() % game.get_game_mode())
            .str();
  ...
  for (auto it = writers.begin(); it != writers.end(); ++it) {
    ...
    if (!out) {
      logger.writef(log_level_t::error,
                    "Something went wrong with opening file: %1%\n", file_name);
      return EX_SOFTWARE;
    }
    it->second->finish();
    it->second->write(out);
...
int process_replay_file(const po::variables_map &vm, const std::string &input,
                        const std::string &output, const std::string &type,
                        bool debug) {
...
int main(int argc, const char *argv[]) {
  ...
  if (vm.count("root") > 0 && chdir(root.c_str()) != 0) {
    logger.writef(log_level_t::error,
                  "Cannot change working directory to: %1%\n", root);
    std::exit(0);
  }

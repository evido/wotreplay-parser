...
CipherContext::CipherContext(const char *cipher_name,
                             const unsigned char *key_data, int key_size,
  ...
  OPENSSL_assert(EVP_CipherInit_ex(ctx, cipher, nullptr, nullptr, nullptr, 0) ==
                 1);
  OPENSSL_assert(EVP_CIPHER_CTX_set_key_length(ctx, key_size) == 1);
  OPENSSL_assert(EVP_CipherInit_ex(ctx, nullptr, nullptr, key_data, iv, 0) ==
                 1);
  OPENSSL_assert(EVP_CIPHER_CTX_set_padding(ctx, 0) == 1);
}

int CipherContext::update(unsigned char *out, int *out_len,

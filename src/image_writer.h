...
namespace wotreplay {
/**
 * wotreplay::image_writer_t draws an image from wotreplay::packet_t on a
 * minimap
 */
...
public:
  ...
  /**
   * When true, colors of positions will not follow the color of the recorder.
   * Team 1 will be colored green, team 2 will be colored red.
...
protected:
   ...
   * @param game Game
   */
  void draw_position(const packet_t &packet, const game_t &game,
   ...
   */
  void draw_element(const boost::multi_array<uint8_t, 3> &element,
                    std::tuple<float, float> position, int mask = 0xFFFFFFFF);
  /**
   * draw an element with coordinates in 'image' space
   * @param element element the element to draw
   ...
   * @param mask the mask to apply to the rgba value
   */
  void draw_element(const boost::multi_array<uint8_t, 3> &element, int x, int y,
                    int mask);
  /**

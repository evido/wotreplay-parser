...
BOOST_FUSION_ADAPT_STRUCT(operation_t, (operator_t, op)(operand_t, left)(operand_t, right))

BOOST_FUSION_ADAPT_STRUCT(draw_rule_t, (uint32_t, color)(operation_t, expr))
...
template <typename Iterator> struct draw_rules_grammar_t : qi::grammar<Iterator, std::vector<draw_rule_t>(), ascii::space_type> {
    draw_rules_grammar_t() : draw_rules_grammar_t::base_type(rules) {
        ...
        // define operator
        operators.add("=", operator_t::EQUAL)("!=", operator_t::NOT_EQUAL)(">=", operator_t::GREATER_THAN_OR_EQUAL)(">", operator_t::GREATER_THAN)(
            "<=", operator_t::LESS_THAN_OR_EQUAL)("<", operator_t::LESS_THAN);

        // define logical operators
        logical_operators.add("and", operator_t::AND)("or", operator_t::OR);

        // define symbols
        symbols.add("player", symbol_t::PLAYER)("clock", symbol_t::CLOCK)("team", symbol_t::TEAM)("tank_tier", symbol_t::TANK_TIER)(
            "tank_class", symbol_t::TANK_CLASS)("tank_icon", symbol_t::TANK_ICON)("tank_country", symbol_t::TANK_COUNTRY)("tank_name", symbol_t::TANK_NAME);

        rules = rule[push_back(_val, _1)] >> *(';' >> rule[push_back(_val, _1)]);
        ...
        expression = (operation[at_c<1>(_val) = _1] >> logical_operators[at_c<0>(_val) = _1] >> expression[at_c<2>(_val) = _1]) |
                     (operation[at_c<1>(_val) = _1] >> logical_operators[at_c<0>(_val) = _1] >> operation[at_c<2>(_val) = _1]) | (operation[_val = _1]);

        operation = operand[at_c<1>(_val) = _1] > operators[at_c<0>(_val) = _1] > operand[at_c<2>(_val) = _1];
        ...
        BOOST_SPIRIT_DEBUG_NODES((expression)(operation)(operand)(value));
...
class printer_t : public boost::static_visitor<void> {
  public:
     ...
     * @param rules rules
     */
    printer_t(const std::vector<draw_rule_t> &rules) : rules(rules) {}
     ...
     */
    void operator()(std::string str) { logger.writef(log_level_t::debug, "'%1%'", str); }

    /**
     * Print nil value
     * @param nil nil value
     */
    void operator()(nil_t nil) { logger.write(log_level_t::debug, "(nil)"); }
...
virtual_machine_t::virtual_machine_t(const game_t &game, const std::vector<draw_rule_t> &rules) : rules(rules), game(game) {}
...
bool virtual_machine_t::operator()(const draw_rule_t rule) { return (*this)(rule.expr) == "true"; }

std::string virtual_machine_t::operator()(nil_t nil) { return "nil"; }

std::string virtual_machine_t::operator()(std::string str) { return str; }
...
std::string virtual_machine_t::operator()(symbol_t symbol) {
    ...
    case symbol_t::TEAM:
        return p->has_property(property_t::player_id) ? boost::lexical_cast<std::string>(game.get_team_id(p->player_id())) : "";
    case symbol_t::CLOCK:
        return p->has_property(property_t::clock) ? boost::lexical_cast<std::string>(p->clock()) : "";

...
namespace wotreplay {
enum game_title_t { world_of_tanks, world_of_warships };
...
public:
  ...
  /**
   * Returns the data block 'replay' containing a sequence of packets describing
   * the actions in the game. This method is not supported for replays before
   * version 0.7.2.
  ...
  /**
   * Tries to find the required property which is the closest (with respect to
   * \c clock) to the specified packet. This method uses the properties
   * \c property::player_id and \c property_t::clock from the packet referenced
   * by packet_id.
   ...
   * @return \c true if a packet was found, \c false if no packet with the
   * required properties was found
   */
  bool find_property(uint32_t clock, uint32_t player_id, property_t property,
                     packet_t &out) const;
  /**
  ...
  /**
   * Returns the data block 'game begin' containing a JSON string describing the
   * start of the game.
   * @return Data block 'game begin'
   */
  const buffer_t &get_game_begin() const;
  /**
   * Returns the data block 'game end' containing a JSON string describing the
   * end of the game. . This method is not supported for replays before version
   * 0.7.2.
...
/**
 * @fn std::tuple<float, float> get_2d_coord(const std::tuple<float, float,
 * float> &position, const bounding_box_t &bounding_box, int width, int height)
 * @brief Get 2D Coordinates from a WOT position scaled to given width and
 * height.
...
get_2d_coord(const std::tuple<float, float, float> &position,
             const bounding_box_t &bounding_box, int width, int height);

/**
 * @fn void wotreplay::show_map_boundaries(const game_t &game, const
 * std::vector<packet_t> &packets) Prints the position boundaries reached by any
 * player in the map.
 * @param game game object containing the team information
 * @param packets a list of all the packets in a replay
 */
void show_map_boundaries(const game_t &game,
                         const std::vector<packet_t> &packets);

/**
 * @fn int wotreplay::get_start_packet(const game_t &game, double skip)
 * Determine offset of first packet a number of seconds after the game has
 * started
...
/**
 * @fn double wotreplay::dist(const std::tuple<float, float, float> &begin,
 * const std::tuple<float, float, float> &end) Calculate the distance between
 * two points

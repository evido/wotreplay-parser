...
class game_t {
  ...
  public:
    ...
    /**
     * Returns the data block 'replay' containing a sequence of packets
     * describing the actions in the game. This method is not supported for
     * replays before version 0.7.2.
     ...
     * \c property::player_id and \c property_t::clock from the packet
     * referenced by packet_id.
    ...
    /**
     * Returns the data block 'game begin' containing a JSON string describing
     ...
     * end of the game. . This method is not supported for replays before
     * version 0.7.2.
 ...
 * @return distance
 */
double dist(const std::tuple<float, float, float> &begin, const std::tuple<float, float, float> &end);
} // namespace wotreplay

...
static std::map<int, std::vector<std::tuple<float, float>>>
getTeamPositions(XMLElement *teamPointsNode) {
  std::map<int, std::vector<std::tuple<float, float>>> team_positions;
  for (XMLElement *node = teamPointsNode->FirstChildElement(); node;
       node = node->NextSiblingElement()) {
    int team = boost::lexical_cast<int>(
        node->Name()[std::strlen((const char *)node->Name()) - 1]);
    std::vector<std::tuple<float, float>> positions;
    for (XMLElement *positionNode = node->FirstChildElement(); positionNode;
         positionNode = positionNode->NextSiblingElement()) {
      std::string nodeName((const char *)positionNode->Name());
      std::tuple<float, float> position =
          getCoordinate((const char *)positionNode->GetText());
...
static bounding_box_t get_bounding_box(XMLElement *boundingBoxNode) {
  ...
  for (XMLElement *node = boundingBoxNode->FirstChildElement(); node;
       node = node->NextSiblingElement()) {
...
static arena_configuration_t
getArenaConfiguration(XMLElement *gameplayTypeNode) {
  arena_configuration_t configuration;
  configuration.mode = (const char *)gameplayTypeNode->Name();
  for (XMLElement *node = gameplayTypeNode->FirstChildElement(); node;
       node = node->NextSiblingElement()) {
    ...
    if (!std::strcmp((const char *)node->Name(), "controlPoint")) {
      configuration.control_point =
          getCoordinate((const char *)node->GetText());
    }
  }
  return configuration;
}

static std::map<std::string, arena_configuration_t>
getArenaConfigurations(XMLElement *gameplayTypesNode) {
  std::map<std::string, arena_configuration_t> configurations;
  for (XMLElement *node = gameplayTypesNode->FirstChildElement(); node;
       node = node->NextSiblingElement()) {
...
static std::map<std::string, arena_t> get_arena_definitions() {
  ...
  for (boost::filesystem::directory_iterator it("maps/definitions");
       it != end_itr; ++it) {
    // Skip if not a file

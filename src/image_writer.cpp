...
image_writer_t::image_writer_t()
    : filter([](const packet_t &) { return true; }), image_width(512),
      image_height(512) {}

void image_writer_t::draw_element(const boost::multi_array<uint8_t, 3> &element,
  ...
  for (int i = 0; i < shape[0]; ++i) {
    for (int j = 0; j < shape[1]; ++j) {
      ...
      for (int k = 0; k < 3; ++k) {
        ...
        base[i + y][j + x][k] =
            mix(base[i + y][j + x][k], base[i + y][j + x][k],
                (255 - element[i][j][3]) / 255.f,
                element[i][j][k] & ((mask >> ((4 - (k + 1)) * 8)) & 0xFF),
                element[i][j][3] / 255.f);
      }
    }
  }
...
image_writer_t::get_element(const std::string &name) {
  boost::multi_array<uint8_t, 3> element, resized_element;
  std::ifstream is("elements/" + name + ".png", std::ios::binary);
  read_png(is, element);
  double f =
      image_width / 512.0; // we need to scale the elements to the right size
  resize(element, element_size * f, element_size * f, resized_element);
  return resized_element;
}

void image_writer_t::draw_element(const boost::multi_array<uint8_t, 3> &element,
                                  std::tuple<float, float> position, int mask) {
  auto shape = base.shape();
  auto element_shape = element.shape();
  float x, y;
  auto position3d =
      std::make_tuple(std::get<0>(position), 0.f, std::get<1>(position));
  std::tie(x, y) = get_2d_coord(position3d, arena.bounding_box, (int)shape[1],
                                (int)shape[0]);
  draw_element(element, x - element_shape[1] / 2, y - element_shape[0] / 2,
               mask);
}
...
void image_writer_t::draw_grid(boost::multi_array<uint8_t, 3> &image) {
  ...
  int grid_width =
      ((int)shape[1] - (grid_cells + 1) * grid_line_width) / grid_cells;
  int grid_height =
      ((int)shape[0] - (grid_cells + 1) * grid_line_width) / grid_cells;
...
void image_writer_t::draw_elements() {
  ...
  if (game_mode != "") {
    ...
    if (it == arena.configurations.end()) {
      wotreplay::logger.writef(
          log_level_t::warning,
          "Could not find configuration for game mode '%1%'\n", game_mode);
      return;
    }

    const arena_configuration_t &configuration =
        arena.configurations[game_mode];
    ...
    for (const auto &entry : configuration.team_base_positions) {
      for (const auto &position : entry.second) {
        draw_element((entry.first - 1) == reference_team_id ? friendly_base
                                                            : enemy_base,
                     position);
      }
    }
    ...
    std::vector<boost::multi_array<uint8_t, 3>> spawns{
        get_element("neutral_spawn1"), get_element("neutral_spawn2"),
        get_element("neutral_spawn3"), get_element("neutral_spawn4")};

    for (const auto &entry : configuration.team_spawn_points) {
      for (int i = 0; i < entry.second.size(); ++i) {
        int mask =
            (reference_team_id == (entry.first - 1)) ? 0x00FF00FF : 0xFF0000FF;
...
void image_writer_t::draw_death(const packet_t &packet, const game_t &game) {
  ...
  bool found = game.find_property(packet.clock(), killed, property_t::position,
                                  position_packet);
  if (found) {
...
void image_writer_t::draw_position(const packet_t &packet, const game_t &game,
                                   boost::multi_array<float, 3> &image) {
  ...
  std::tuple<float, float> position =
      get_2d_coord(packet.position(), bounding_box, width, height);
...
void image_writer_t::write(std::ostream &os) { write_png(os, result); }
...
void image_writer_t::clear() {
  ...
  std::fill(positions.origin(), positions.origin() + positions.num_elements(),
            0.f);
}
...
bool image_writer_t::is_initialized() const { return initialized; }

void image_writer_t::set_show_self(bool show_self) {
  this->show_self = show_self;
}

bool image_writer_t::get_show_self() const { return show_self; }
...
int image_writer_t::get_recorder_team() const { return recorder_team; }

void image_writer_t::set_filter(filter_t filter) { this->filter = filter; }

const arena_t &image_writer_t::get_arena() const { return arena; }

const std::string &image_writer_t::get_game_mode() const { return game_mode; }

void image_writer_t::merge(const image_writer_t &writer) {
  // TODO: implement this
  std::transform(
      positions.origin(), positions.origin() + positions.num_elements(),
      writer.positions.origin(), positions.origin(), std::plus<float>());
}

int image_writer_t::get_image_height() const { return image_height; }

void image_writer_t::set_image_height(int image_height) {
  this->image_height = image_height;
}

int image_writer_t::get_image_width() const { return image_width; }

void image_writer_t::set_image_width(int image_width) {
  this->image_width = image_width;
}

bool image_writer_t::get_no_basemap() const { return no_basemap; }

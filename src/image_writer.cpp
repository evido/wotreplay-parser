...
image_writer_t::image_writer_t() : filter([](const packet_t &) { return true; }), image_width(512), image_height(512) {}
...
void image_writer_t::draw_elements() {
    ...
    if (game_mode != "") {
        ...
        std::vector<boost::multi_array<uint8_t, 3>> spawns{get_element("neutral_spawn1"), get_element("neutral_spawn2"), get_element("neutral_spawn3"),
...
void image_writer_t::set_show_self(bool show_self) { this->show_self = show_self; }

bool image_writer_t::get_show_self() const { return show_self; }

void image_writer_t::set_use_fixed_teamcolors(bool use_fixed_teamcolors) { this->use_fixed_teamcolors = use_fixed_teamcolors; }

bool image_writer_t::get_use_fixed_teamcolors() const { return use_fixed_teamcolors; }

void image_writer_t::set_recorder_team(int recorder_team) { this->recorder_team = recorder_team; }
...
void image_writer_t::merge(const image_writer_t &writer) {
    // TODO: implement this
    std::transform(positions.origin(), positions.origin() + positions.num_elements(), writer.positions.origin(), positions.origin(), std::plus<float>());
}

int image_writer_t::get_image_height() const { return image_height; }

void image_writer_t::set_image_height(int image_height) { this->image_height = image_height; }

int image_writer_t::get_image_width() const { return image_width; }

void image_writer_t::set_image_width(int image_width) { this->image_width = image_width; }

bool image_writer_t::get_no_basemap() const { return no_basemap; }

void image_writer_t::set_no_basemap(bool no_basemap) { this->no_basemap = no_basemap; }
...
const boost::multi_array<uint8_t, 3> &image_writer_t::get_result() const { return result; }

...
/**
 * writer_t describes the interfaces for all classes with the ability to create
 * an output from game_t objects.
 */
class writer_t {
  public:
    /**
     ...
     * or void writer_t::finish() again after this method. The only valid
     * methods are void writer_t::reset() and void image_t::write_image(const
     * std::string &path);
     */
    virtual void finish() = 0;
    /**
     * Empties modifications made by writer_t::update(const game_t &game). After
     * calling this method, the object should find itself in the state after
     * calling init(map, mode)
     */

...
/**
 * @fn template<typename U, typename T> const U & wot::get_field(T begin, T end,
 * size_t offset)
...
public:
  ...
  const std::array<bool, static_cast<size_t>(property_t::property_nr_items)> &
   ...
   */
  template <typename T> bool find(const T &value) const {
    const uint8_t *arr = reinterpret_cast<const uint8_t *>(&value);
    return std::search(data.begin(), data.end(), arr, arr + sizeof(value)) !=
           data.end();
  }
  /**
   ...
   * example: \verbatim <font color='#DA0400'>SmurfingBird[RDDTX] (VK 36.01
   * H)&nbsp;:&nbsp;</font><font color='#FFFFFF'>so far so good</font>
   * \endverbatim
  ...
  /**
   * get status of alternative track, 0xFO indicates the track works, 0xF6
   * indicates the track is broken
   * @return alt track state
   */
  uint8_t alt_track_state() const;
  std::string map_name() const;

  template <typename T> const T &get_data_field(size_t offset) const {
    return wotreplay::get_field<T>(data.begin(), data.end(), offset);
  }

private:
  /** An array containing the presence of each property. */
  std::array<bool, static_cast<size_t>(property_t::property_nr_items)>
      properties;
 ...
 * specified packet. This method uses the properties
 * \c property::player_id and \c property_t::clock from the packet referenced by
 * packet_id.
 ...
 */
bool find_property(const std::vector<packet_t> &packets, uint32_t clock,
                   uint32_t player_id, property_t property, packet_t &out);

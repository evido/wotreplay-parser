...
class logger_t {
  public:
    /**
     * Create new instance of logger, logger will write to the given output
     * stream
     ...
     */
    template <typename... Args> void writef(log_level_t level, const std::string &format, const Args &...args) {
     ...
     * @param args formatting string paramaters
     */
    template <typename... Args> void writef(const std::string &_format, const Args &...args) {
  ...
  private:
    ...
    template <typename T, typename... Args> void writef(boost::format &format, const T &val, const Args &...args) { writef(format % val, args...); }

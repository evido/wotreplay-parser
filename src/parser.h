...
class parser_t {
   ...
   * @param path The directory which contains the replays to validate the parser
   * with
   */
...
public:
   ...
   */
  parser_t(std::unique_ptr<packet_reader_t> &&packet_reader,
           load_data_mode_t load_data_mode = load_data_mode_t::bulk,
           bool debug = false);
  /**
   * Indicates if the parsed file is a legacy (< 0.7.2) replay file. This means
   * 'game begin', 'game end' will be missing.
...
private:
   ...
   */
  void get_data_blocks(buffer_t &buffer,
   ...
   */
  void decrypt_replay(unsigned char *begin, unsigned char *end,
   ...
   * @param replay The output variable that will contain the decompressed replay
   * after the execution of this method.
   */
  void extract_replay(const unsigned char *begin, const unsigned char *end,
                      buffer_t &replay);
  /**

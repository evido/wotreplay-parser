...
class parser_t {
     ...
     * @param path The directory which contains the replays to validate the
     * parser with
     */
  ...
  public:
    ...
    /**
     * Indicates if the parsed file is a legacy (< 0.7.2) replay file. This
     * means 'game begin', 'game end' will be missing.
     * @return \c true if file is in a legacy format \c false if the file is in
     * the 'new' format.
     */
     ...
     * @param game The output variable containing the parsed contents of the
     * replay file.
     */
  ...
  private:
    ...
    /**
     * Extracts each data block from the replay file and adds it the the output
     * variable.
     * @param buffer A buffer_t containing the complete contents of a replay
     * file.
     * @param data_blocks output variable to contain a slice_t to each data
     * block.
     */
    ...
    /**
     * Performs an in place decryption of the replay using the given key. The
     * decryption is a (broken) variant of CBC decryption and is performed as
     * follows:
    ...
    /**
     * This method performs an extraction (using zlib) of the given replay. The
     * resulting data is copied to a new buffer.
     * @param begin start of buffer to inflate
     * @param end end of buffer to inflate
     * @param replay The output variable that will contain the decompressed
     * replay after the execution of this method.
     */
     ...
     */
    template <typename iterator> packet_t read_packet(iterator begin, iterator end);
    /**
     * Helper method to read all the packets contained in the buffer
     * replay_file::replay.
     * @param game Output variable game
     */
    void read_packets(game_t &game);
    /**
     * Extracts a game_info object from either the 'game begin' data block or
     * parsed directly from the 'replay' data block.
     * @param game Output variable game
     */
...
template <typename iterator> packet_t parser_t::read_packet(iterator begin, iterator end) { return packet_t(boost::make_iterator_range(begin, end)); }
 ...
 * @return @c true if the path is expected to be a valid replay file, @c false
 * if not
 */

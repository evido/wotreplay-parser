...
static std::map<uint8_t, packet_config_t> base_packet_configs = {
    ...
    {0x1F, {17,  8}},
    {0x20, {21, 13}},  // modified for 0.8.0 {0x20, 4}
...
void packet_reader_80_t::init(const version_t &version, buffer_t *buffer)
{
    ...
    if (start != buffer->end()) {
        pos = static_cast<int>(std::distance(buffer->begin(), start) + marker.size()) + 1;
        wotreplay::log.write(log_level_t::debug, (boost::format("OFFSET: %1%\n") % pos).str());
    }
}

packet_t packet_reader_80_t::next() {
    auto it = packet_configs.find((*buffer)[pos]);
    if (it == packet_configs.end()) {
        throw std::runtime_error((boost::format("no such packet type 0x%1$02X") % (*buffer)[pos]).str());
    }
    ...
    // include 25 byte ?
    if ((pos + 24 + total_packet_size) > buffer->size()) {
...
bool packet_reader_80_t::has_next() {
    ...
    return (pos + 24) < buffer->size();
}

bool packet_reader_80_t::is_compatible(const version_t &version) {
    return version.major == 8 && (version.minor >= 0 && version.minor <= 8);
}